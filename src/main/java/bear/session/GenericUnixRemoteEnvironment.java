...
//            }
    ...
    public void connect() {
        if (sshSession == null) {
            sshAddress.username = $.var(bear.sshUsername);
            sshAddress.password = $.var(bear.sshPassword);
    ...
    public static GenericUnixRemoteEnvironment newUnixRemote(String name, String address, GlobalContext g) {
        return newUnixRemote(name, g.var(g.bear.sshUsername), g.var(g.bear.sshPassword), address, g);
    }
